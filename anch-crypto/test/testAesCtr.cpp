...
main(int argc, char** argv) {
  ...
  if(all || (strcmp("192", argv[1]) == 0 && strcmp("sequential", argv[2]) == 0)) {
    ...
    std::cout << "Compute Makefile.ctr.aes192.decipher hash using SHA1" << std::endl;
    input = new std::ifstream("Makefile.ctr.aes192.decipher", std::ifstream::binary);
    SHA1 cipherHash(*input);
    std::array<uint8_t,20> resHash = cipherHash.digest();
    input->close();
    delete input;
  ...
  if(all || (strcmp("192", argv[1]) == 0 && strcmp("parallel", argv[2]) == 0)) {
    ...
    std::cout << "Compute Makefile.ctr.aes192.async.decipher hash using SHA1" << std::endl;
    input = new std::ifstream("Makefile.ctr.aes192.async.decipher", std::ifstream::binary);
    SHA1 cipherHash(*input);
    std::array<uint8_t,20> resHash = cipherHash.digest();
    input->close();
    delete input;
